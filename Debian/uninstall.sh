...
# ~/.cache/tooluniverse and the ToolRAG-T1 embedder (5.75 GiB) in the
...
aistackUninstallTooluniverseTools() {
    ...
    for c in "${XDG_CACHE_HOME:-$HOME/.cache}/tooluniverse" \
...
aistackUninstall() {
    ...
    if [ "$left" = "1" ]; then
        ...
        warn "Engines, agents, tools, monitoring and the foundations are all kept — models depend on them."
