...
#                  tokens-per-second. Debian/Ubuntu implementation.
#
...
# and, because nothing here is platform-specific, comparable with the macOS
# numbers from the same models too.
#
# Usage:
...
# Args: <engine> <model>. Ollama loads on demand; llama.cpp binds one model at
# server start, so a server on a different model is restarted.
    ...
    case "$engine" in
        Ollama)
            ...
            OLLAMA_MODELS="$(ollamaModelsDir)" \
                ...
                nohup ollama serve >"${TMPDIR:-/tmp}/ollama-serve-test.log" 2>&1 &
            TEST_SERVER_PID=$!
            sleep 3
            engine_up Ollama "$host" || { fail "Ollama did not start — see ${TMPDIR:-/tmp}/ollama-serve-test.log"; return 1; }
            ;;
        Llama.cpp)
            ...
            if engine_up Llama.cpp "$host"; then
                ...
                # matched by OUR port, never by the bare name
                llamacppKillOurs; sleep 2
            fi
            while IFS= read -r a; do extra+=("$a"); done < <(llamacppServerArgs)
            info "Starting llama-server with $(basename "$f") ($(llamacppBackend) backend)..."
        ...
        *)
            ...
            fail "No test path for engine '${engine}'."
            return 1
            ;;
    esac
}
...
# Args: <engine> <model>. SKIP rather than FAIL on llama.cpp: it serves an
# OpenAI-compatible API by design, so this is a capability note, not a defect.
# Uses a generous token budget — thinking models reason before replying.
...
try:
    ...
    print(d.get("default_generation_settings",{}).get("n_ctx", d.get("n_ctx",0)) or 0)
            ...
            ;;
        *)
            warn "${engine} exposes no context endpoint — cannot verify."
            R_context=WARN
            return 0
            ;;
    esac
