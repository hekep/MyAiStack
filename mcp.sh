...
# aistackMcpLogin) and no tool shims (that is aistackMcpTest).
aistackMcpAdd() {
    if [ $# -lt 3 ] || [ "$2" != "--url" ]; then
...
# ---------- tools -------------------------------------------------------------
# Verify a connector end to end and cache what it offers. Args: <name>.
...
# completion — no JSON parsing on the completion path). Then regenerates the
# shims for whichever agents were chosen at add time.
aistackMcpTest() {
    if [ $# -lt 1 ]; then
        aiStackUsage "aistackMcpTest <name>" \
            "example : aistackMcpTest $(_aiStackMcpFirstName || echo aidlab)"
        return 2
    fi
    local name="$1" dir n
    _aiStackMcpRequire "$name" || return 1
    ...
    info "aistackMcpTest — handshake and tool discovery for ${name}"
...
aistackMcpTools() {
    ...
    _aiStackMcpRequire "$name" || return 1
    dir=$(_aiStackMcpDir "$name")
    [ -f "$dir/tools.json" ] || { fail "No tools cached for '${name}' — run: aistackMcpTest ${name}"; return 1; }
...
# Called at the end of aistackMcpTest, so shims always match the tools the
...
// GENERATED by aistackMcpTest — do not edit; change the template in mcp.sh.
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_e, ctx) => {
    try {
      ...
      ctx.ui.notify(\`\${LABEL}: \${e.message} — run: aistackMcpTest \${NAME}\`, "warning");
    }
  });
      ...
      if (args.trim() === "reload") {
        try {
          await new Promise<void>((res, rej) =>
            execFile("bash", ["-c", '. "\$0" && aistackMcpTest "\$1"', MCP_SH, NAME], e => e ? rej(e) : res()));
...
for t in tools:
    ...
    body = f'''// GENERATED by aistackMcpTest — do not edit; change the template in mcp.sh.
...
PY
            ...
            warn "No Pi extension generated yet — run: aistackMcpTest ${name}"
        fi ;;
    esac
    ...
    case " $agents " in *" OpenCode "*)
        ...
        if [ "$mode" = "on" ]; then
            [ -d "$dir/opencode-tools" ] || { warn "No OpenCode tools generated yet — run: aistackMcpTest ${name}"; return 0; }
            mkdir -p "$base"
